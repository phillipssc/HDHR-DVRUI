...
if [[ "$fbas" =~ (.*)\ (.*)\ (.*) ]]; then
  ...
  #step2a comskip in place and exit if comskip no archive
  if [[ "${comskip}" != "N" ]] && [[ "${archive}" == "N" ]]; then
    echo comskip | tee -a "${temp_dir}${fbas}.log"
    eval "${comcut_app} --comskip=${comskip_app} --comskip-ini=${comskipini} \"$1\"" &>> "${temp_dir}${fbas}.log"
    rm "${fdir}*.ccyes"
    exit 1
  fi
